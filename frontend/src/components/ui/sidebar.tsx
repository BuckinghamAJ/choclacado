    ...
    <Switch>
      ...
      <Match when={isMobile()}>
        <Sheet open={openMobile()} onOpenChange={setOpenMobile} {...others}>
          <SheetContent
            ...
            data-mobile="true"
            class="w-(--sidebar-width) shadow-none bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
            style={{
      ...
      <Match when={!isMobile()}>
        ...
        >
          ...
          <div
            class={cn(
              "w-(--sidebar-width) fixed inset-y-0 z-10 hidden h-svh transition-[left,right,width] bg-white duration-200 ease-linear md:flex",
              local.side === "left"
                ? "left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]"
                : "right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)+32rem]",
          ...
          >
            <div
              data-sidebar="sidebar"
              class="flex size-full shadow-none! flex-col bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
            >
              {local.children}
            </div>
          </div>
